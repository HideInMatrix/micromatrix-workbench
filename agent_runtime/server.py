...
HEALTH_PATH = "/.well-known/micromatrix-workbench-health"
...
class MCPHTTPServer(http.server.ThreadingHTTPServer):
    # ``TCPServer`` defaults to a backlog of only 5. Public tunnels create
    # short-lived origin connections in bursts, so that tiny queue can turn a
    # healthy Runtime into an upstream 502 before Python accepts the socket.
...
class MCPHandler(http.server.BaseHTTPRequestHandler):
    ...
    # Streamable HTTP is request/response framed with Content-Length. HTTP/1.1
    # lets reverse tunnels reuse origin connections instead of creating one
    # short-lived TCP connection per MCP request, reducing transient upstream
    # 502s under bursty tool traffic.
    ...
    def _post_internal_error(self, exc: Exception) -> None:
        ...
        LOGGER.error(
            "Unhandled HTTP POST failure",
            exc_info=(type(exc), exc, exc.__traceback__),
        )
        raw_path = urllib.parse.urlparse(self.path).path.rstrip("/")
        is_mcp = raw_path == ENDPOINT_PATH or raw_path.endswith(ENDPOINT_PATH)
        ...
        try:
            if is_mcp:
                ...
                return
            self._json(500, {"error": "internal_error"}, {"Connection": "close"})
        except (BrokenPipeError, ConnectionResetError, ConnectionAbortedError):
            self.close_connection = True

    def end_headers(self) -> None:
        # A 502 generated by Cloudflare/ngrok/FRP will not contain this header;
        # its presence indicates that an Agent Runtime shaped the response.
        self.send_header(ORIGIN_HEADER, ORIGIN_HEADER_VALUE)
        super().end_headers()
    ...
    def _json(self, status: int, payload: Any, headers: dict[str, str] | None = None) -> None:
        ...
        if status >= 400:
            self._log_error_response(status, payload)
        ...
        self.end_headers()
        self._write_response_body(body, status)
    ...
    def _html(self, status: int, body: str) -> None:
        ...
        self.end_headers()
        self._write_response_body(data, status)

    def _log_error_response(self, status: int, payload: Any) -> None:
        ...
        protocol = self.headers.get("MCP-Protocol-Version", "").strip()
        content_length = self.headers.get("Content-Length", "").strip()
        print(
            "[http] response_error "
            f"status={status} method={self.command} path={request_path} "
            f"rpc_code={code or '-'} reason={reason or '-'} "
            f"content_length={content_length or '-'} protocol={protocol or '-'} "
            f"message={message or '-'}",
            file=sys.stderr,
        )

    def _write_response_body(self, body: bytes, status: int) -> None:
        try:
            self.wfile.write(body)
        except (BrokenPipeError, ConnectionResetError, ConnectionAbortedError):
            # The caller or an intermediary cancelled after dispatch. This is
            # not a Runtime crash and retry policy belongs to the client.
            self.close_connection = True
            print(
                "[http] response_cancelled "
                f"status={status} method={self.command} "
                f"path={urllib.parse.urlparse(self.path).path}",
                file=sys.stderr,
            )
    ...
    def _read(self, limit: int) -> bytes:
        ...
        if len(body) != length:
            ...
            print(
                ...
                f"method={self.command} path={urllib.parse.urlparse(self.path).path} "
                f"expected_bytes={length} received_bytes={len(body)}",
                file=sys.stderr,
            )
            raise ConnectionAbortedError("request body ended before Content-Length")
        return body
