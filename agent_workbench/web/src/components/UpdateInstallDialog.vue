    ...
    <DialogPortal>
      ...
      >
        ...
        <div v-if="impact?.services.length" class="mt-4 rounded-md border border-border bg-secondary p-3">
          <p class="m-0 text-xs font-medium">将停止以下 Work：</p>
          ...
          <p class="mt-2 mb-0 text-[11px] text-muted-foreground">这些 Work 的 Runtime 和连接会一并停止；已启用的 Work 会在应用重新启动后自动恢复。</p>
        </div>
        <p v-else class="mt-4 text-xs text-muted-foreground">当前没有运行中的 Work。</p>
