...
Workbench 不再在一个本地端口中按 URL Path 或 Hostname 分发多个 Runtime。每个 Work 都是独立服务：使用自己的本地端口、Runtime 与公网域名。固定 Cloudflare 地址时，每个 Work 也必须配置独立 Named Tunnel 和独立 Tunnel Token；Work 列表上的 Switch 决定该 Work 是否随应用启动。
