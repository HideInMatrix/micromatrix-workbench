...
## 10. Cloudflare 固定 Hostname 与 Local Gateway Host Routing
...
- 同一台电脑的多 Workspace Service 允许在一个 Named Tunnel 中配置多个独立 hostname，并全部回源到同一个 `127.0.0.1:<port>`。
- Cloudflare Published Application 的 Path 保持为空；Profile 选择由 Gateway 根据 HTTP Host 完成。
...
- 每个多 Workspace Profile 必须配置独立 Public Hostname，且 hostname 在全部 Direct Server / Gateway 中不得冲突。
- 每个 Profile 对外 MCP resource 固定为 `https://<profile-host>/mcp`。
- 每个 Profile 的 Authorization Server issuer 固定为 `https://<profile-host>`，authorize/token/register 使用根 `/oauth/*`。
- RFC 9728 Protected Resource Metadata 固定为 `https://<profile-host>/.well-known/oauth-protected-resource/mcp`。
- Server Card `transport.endpoint` 必须为 `/mcp`，不能暴露 legacy `instance_path`。
- Gateway 必须维护 `Host -> server_id` 路由；未知公网 Host 必须返回 404，禁止回退到主 Workspace。
- legacy `instance_path` 路由仅用于旧配置和本机兼容，不得作为新 Profile 公网身份。
- 不同 Hostname Profile 必须使用不同 OAuth issuer 存储目录、Registry 和 token secret，Gateway Profile 状态不得互相共享。
- Service 必须显式保存 `single | multi` 运行模式；Profile 数量不得自动改变运行模式。`single` 只启动主 Workspace，`multi` 才启动全部 Hostname Profile。
- 从 `multi` 切回 `single` 时，子 Profile 配置必须保留，但不得创建 Runtime 或 OAuth active session；再次切回 `multi` 后恢复参与运行。
- Gateway route probe 必须返回非敏感 Workspace fingerprint，用于确认每个公网 Hostname 实际落到正确 Runtime；不得返回真实 Workspace Path。
- Cloudflare Named Tunnel 启动后的多 Workspace E2E 自检失败不得关闭已经健康运行的服务/Tunnel，只记录告警并允许用户在服务页面手动重试。
- Gateway E2E 必须至少覆盖：本地兼容 Path→Runtime、公网 Hostname→Runtime、Server Card、Authorization Server Metadata、Protected Resource Metadata、MCP 401 `resource_metadata` challenge，以及真实 Authorization Code + PKCE S256 → Token Exchange。
- “OAuth 授权”页面按 Service/Profile 展示；`single` 模式下仅主 Workspace 标记为运行中，已保存但未启用的子 Profile 不得伪装成活动 Runtime。
